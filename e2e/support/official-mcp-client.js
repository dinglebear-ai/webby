...
export class OfficialMCPClient {
  constructor({baseUrl, token, producer, headers = {}, fetchImpl = fetch, deadlines = {}, versionNegotiation = {mode: "legacy"}} = {}) {
    ...
    this.producer = producer; this.deadlines = {...DEFAULT_DEADLINES, ...deadlines}; this.active = new Set(); this.timers = new Set(); this.closed = false
  ...
  recordingFetch(fetchImpl) { return async (url, init = {}) => { const method = init.method ?? "GET"; const body = typeof init.body === "string" ? JSON.parse(init.body) : undefined; await this.producer?.event("official-mcp.request", {method, rpc_method: body?.method, rpc_id: body?.id}); const response = await fetchImpl(url, init); await this.producer?.event("official-mcp.response", {method, rpc_method: body?.method, status: response.status}); return response } }
  ...
  async close() { if (this.closed) return; this.closed = true; for (const controller of this.active) controller.abort("official client closing"); let timer; try { const timeout = new Promise((_, reject) => { timer = setTimeout(() => reject(new Error("official MCP shutdown deadline exceeded")), this.deadlines.shutdown); timer.unref?.(); this.timers.add(timer) }); await Promise.race([(async () => { await this.transport.terminateSession().catch(error => { if (!/405|Method Not Allowed/i.test(error.message)) throw error }); await this.client.close() })(), timeout]) } finally { clearTimeout(timer); this.timers.delete(timer) } }
  handles() { return {active_requests: this.active.size, timers: this.timers.size, transport_closed: this.closed, closed: this.closed} }
}
