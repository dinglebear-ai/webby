...
export class ChromiumScenarioAdapter {
  ...
  async pair({boundary}) {
    ...
    const request = type => events.findLast(event => event.producer === "extension" && event.direction === "out" && event.type === type)
    const incoming = type => events.findLast(event => event.producer === "extension" && event.direction === "in" && event.type === type)
    const reply = outbound => outbound && events.find(event => event.producer === "extension" && event.direction === "in" && event.ref === outbound.ref && event.status === "ok")
    ...
    const pairingApproved = incoming("pairing.approved"), authChallenge = incoming("auth.challenge")
    assert.ok(pairingApproved?.sequence, "pairing.approved producer token is missing")
    assert.ok(authChallenge?.sequence, "auth.challenge producer token is missing")
    observeSurfaceProofs(boundary, {"topic:pairing": surfaceProof.chrome(pairingRequest, {eventName: "pairing.request", identity: pairingRequest.identity}), "in:pairing-request": surfaceProof.protocol(pairingRequest, {direction: "out"}), "out:pairing-pending": surfaceProof.protocol(pairingReply, {direction: "in"}), "in:pairing-status": surfaceProof.protocol(pairingStatus, {direction: "out"}), "out:pairing-status": surfaceProof.protocol(statusReply, {direction: "in"}), "dashboard:approve": surfaceProof.dashboard(this.dashboard.lastOperation), "out:pairing-approved": surfaceProof.protocol(pairingApproved, {direction: "in"}), "topic:auth": surfaceProof.chrome(authResponse, {eventName: "auth.respond", identity: authResponse.identity}), "out:auth-challenge": surfaceProof.protocol(authChallenge, {direction: "in"}), "in:auth-respond": surfaceProof.protocol(authResponse, {direction: "out"}), "out:auth-accepted": surfaceProof.protocol(authReply, {direction: "in"}), "in:browser-hello": surfaceProof.protocol(hello, {direction: "out"}), "out:browser-welcome": surfaceProof.protocol(helloReply, {direction: "in"})})
    boundary.complete()
  ...
  async discover({boundary}) {
    ...
    observeSurfaceProofs(boundary, {"in:browser-resync": surfaceProof.protocol(resync, {direction: "out"}), "in:discovery-observed": surfaceProof.protocol(discoveryObserved, {direction: "out"}), "out:ack": surfaceProof.protocol(acknowledgement, {direction: "in"}), "dashboard:register": surfaceProof.dashboard(this.dashboard.lastOperation)})
    boundary.complete()
  ...
  async credential({boundary}) {
    ...
    observeSurfaceProofs(boundary, {"dashboard:create-credential": surfaceProof.dashboard(this.dashboard.lastOperation)})
    boundary.complete()
