...
const allowedKinds = Object.freeze({
  http: new Set(["http_response", "mcp_exchange", "http_receipt"]), in: new Set(["protocol_event", "mcp_exchange", "fixture_outcome", "in_receipt"]), out: new Set(["protocol_event", "mcp_exchange", "fixture_outcome", "out_receipt"]), topic: new Set(["protocol_event", "chrome_event", "topic_receipt"]),
  mcp: new Set(["mcp_exchange", "fixture_outcome", "mcp_receipt"]), action: new Set(["mcp_exchange", "fixture_outcome", "action_receipt"]), version: new Set(["mcp_exchange", "version_receipt"]), dashboard: new Set(["dashboard_entity", "dashboard_receipt"]),
  "world-field": new Set(["manifest_field", "fixture_outcome", "world-field_receipt"]), capability: new Set(["manifest_field", "chrome_event", "fixture_outcome", "capability_receipt"]), artifact: new Set(["artifact_attestation"]), fixture: new Set(["chrome_event", "fixture_outcome", "fixture_receipt"]),
  "ext-event": new Set(["chrome_event", "fixture_outcome", "ext-event_receipt"]), "chrome-event": new Set(["chrome_event", "chrome-event_receipt"]), storage: new Set(["chrome_event", "storage_receipt"]), behavior: new Set(["http_response", "behavior_receipt"]),
})
for (const kinds of Object.values(allowedKinds)) kinds.add("system_output")
...
export const surfaceProof = Object.freeze({
  ...
  systemOutput(output, {source, identity} = {}) { if (!output || typeof output !== "object" || !hasText(source) || !hasText(String(identity ?? "")) || output.data?.surface_id || output.type === "surface.test.observed") throw new Error("system output proof requires an immutable transport, Chrome, fixture, or persistence result"); const snapshot = structuredClone(output); const output_sha256 = createHash("sha256").update(JSON.stringify(snapshot)).digest("hex"); return deepFreeze({kind: "system_output", source, identity: String(identity), output: snapshot, output_sha256}) },
})
...
export function observeSystemOutputSurfaces(boundary, output, surfaceIds, source, identity) {
  if (!boundary) return
  if (!Array.isArray(surfaceIds) || surfaceIds.length === 0) throw new Error("system output surface IDs are required")
  const proof = surfaceProof.systemOutput(output, {source, identity})
  observeSurfaceProofs(boundary, Object.fromEntries(surfaceIds.map(surfaceId => [surfaceId, proof])))
}
...
export function validateSurfaceProof(surfaceId, proof) {
  ...
  if (proof.kind.endsWith("_receipt")) { const payload = {surface_id: proof.surface_id, correlation: proof.correlation, runtime_nonce: proof.runtime_nonce}; if (proof.kind !== `${category(surfaceId)}_receipt` || !positive(proof.sequence) || !hasText(proof.type) || !hasText(proof.producer) || proof.surface_id !== surfaceId || proof.correlation === null || proof.correlation === undefined || !hasText(proof.runtime_nonce) || !/^[a-f0-9]{64}$/.test(proof.payload_sha256 ?? "") || createHash("sha256").update(JSON.stringify(payload)).digest("hex") !== proof.payload_sha256) throw new Error(`${surfaceId}: typed producer receipt is incomplete or unbound`) }
  ...
  if (proof.kind === "system_output" && (!hasText(proof.source) || !hasText(proof.identity) || !proof.output || proof.output.data?.surface_id || !/^[a-f0-9]{64}$/.test(proof.output_sha256 ?? "") || createHash("sha256").update(JSON.stringify(proof.output)).digest("hex") !== proof.output_sha256)) throw new Error(`${surfaceId}: system output proof is incomplete or fabricated`)
