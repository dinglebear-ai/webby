...
const allowedKinds = Object.freeze({
  ...
  "world-field": new Set(["manifest_field", "journal_event"]), capability: new Set(["manifest_field", "chrome_event", "journal_event"]), artifact: new Set(["artifact_attestation"]), fixture: new Set(["journal_event", "chrome_event"]),
...
export async function observeRecordedSurfaces(boundary, producerOrSurfaceIds, surfaceIdsOrType, typeOrData, data = {}) {
  if (!boundary) return
  if (Array.isArray(producerOrSurfaceIds)) return Promise.all(producerOrSurfaceIds.map(surfaceId => boundary.record(surfaceId, `surface.${surfaceId.replaceAll(":", ".")}.observed`, {correlation: surfaceIdsOrType})))
...
export function createBoundaryObservation(scenarioId, operation, recorder) {
  ...
  return Object.freeze({
    ...
    record(surfaceId, type, data = {}) { if (!recorder?.producers?.world?.event) throw new Error(`${scenarioId}/${operation}: recorder-backed boundary proof is required`); if (sealed) throw new Error(`${scenarioId}/${operation}: boundary evidence is already sealed`); if (!allowed.has(surfaceId)) throw new Error(`${scenarioId}/${operation}: undeclared boundary surface: ${surfaceId}`); if (observed.has(surfaceId) || reserved.has(surfaceId)) throw new Error(`${scenarioId}/${operation}: boundary surface was observed more than once: ${surfaceId}`); reserved.add(surfaceId); const task = Promise.resolve(recorder.producers.world.event(type, {scenario_id: scenarioId, operation, ...data, surface_id: surfaceId})).then(event => { observed.set(surfaceId, validateSurfaceProof(surfaceId, surfaceProof.journal(event, surfaceId))); return event }); pending.add(task); task.finally(() => pending.delete(task)); return task },
