...
const allowedKinds = Object.freeze({
  ...
  "world-field": new Set(["manifest_field", "journal_event"]), capability: new Set(["manifest_field", "chrome_event", "journal_event"]), artifact: new Set(["artifact_attestation"]), fixture: new Set(["chrome_event", "journal_event"]),
...
export const surfaceProof = Object.freeze({
  ...
  mcp(exchange, {method, action, version} = {}) { const actual = exchange?.exchange; if (!exchange || typeof exchange.status !== "number" || !actual || !positive(actual.id) || !hasText(actual.rpc_method) || actual.transport_method !== "POST" || actual.path !== "/mcp") throw new Error("MCP proof requires an immutable request/response exchange"); if (method !== actual.rpc_method && method !== actual.transport_method) throw new Error("MCP method correlation drifted"); if (action !== undefined && action !== actual.action) throw new Error("MCP action correlation drifted"); if (version !== undefined && version !== actual.version) throw new Error("MCP version correlation drifted"); return freeze({kind: "mcp_exchange", exchange_id: actual.id, transport_method: actual.transport_method, method: actual.rpc_method, path: actual.path, status: exchange.status, ...(actual.action === undefined ? {} : {action: actual.action}), ...(actual.version === undefined ? {} : {version: actual.version})}) },
...
export function validateSurfaceProof(surfaceId, proof) {
  ...
  if (proof.kind === "mcp_exchange" && (!positive(proof.exchange_id) || proof.transport_method !== "POST" || proof.path !== "/mcp" || !hasText(proof.method) || typeof proof.status !== "number")) throw new Error(`${surfaceId}: MCP proof is incomplete`)
