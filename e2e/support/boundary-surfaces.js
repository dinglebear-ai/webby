...
export const surfaceProof = Object.freeze({
  ...
  producerReceipt(event, surfaceId) { const surfaceCategory = category(surfaceId); if (!eventShape(event) || !hasText(surfaceId) || !hasText(event.producer) || !event.data || event.data.surface_id !== surfaceId || !Object.hasOwn(event.data, "correlation") || event.data.correlation === null || event.data.correlation === undefined || !hasText(event.data.runtime_nonce)) throw new Error("typed producer receipt requires a surface-bound runtime event and nonce"); const payload = {surface_id: surfaceId, correlation: event.data.correlation, runtime_nonce: event.data.runtime_nonce}; return deepFreeze({kind: `${surfaceCategory}_receipt`, sequence: event.sequence, type: event.type, producer: event.producer, ...payload, payload_sha256: createHash("sha256").update(JSON.stringify(payload)).digest("hex")}) },
})
...
export async function observeRecordedSurfaces(boundary, producer, surfaceIds, type, data = {}) {
  ...
  if (!hasText(data.runtime_nonce)) throw new Error("typed runtime receipts require the live world nonce")
  const events = await Promise.all(surfaceIds.map(surfaceId => producer.event(type, {surface_id: surfaceId, ...data}))); observeSurfaceProofs(boundary, Object.fromEntries(events.map((event, index) => [surfaceIds[index], surfaceProof.producerReceipt(event, surfaceIds[index])]))); return events
}
