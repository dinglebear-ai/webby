...
import {observeRecordedSurfaces, surfaceProof} from "../../support/boundary-surfaces.js"
...
test("instrumented runner rejects a completed action that omitted verified boundary evidence", async t => {
  ...
  const telemetryPath = join(recorder.root, "negative-scenario-telemetry.ndjson")
  ...
  t.after(() => { if (previousTelemetry === undefined) delete process.env.WEBBY_E2E_SCENARIO_TELEMETRY; else process.env.WEBBY_E2E_SCENARIO_TELEMETRY = previousTelemetry; if (previousDenominator === undefined) delete process.env.WEBBY_E2E_SCENARIO_DENOMINATOR; else process.env.WEBBY_E2E_SCENARIO_DENOMINATOR = previousDenominator })
  ...
  const runner = new ScenarioRunner({scenario: scenarioTemplate, driver: "protocol", world: {worldId: "world-unit", instanceNonce: "nonce", seed: 1}, recorder, actions, observe: async () => ({}), cleanup, recordTelemetry: false})
  await assert.rejects(runner.run(), /verified boundary completion evidence is missing/)
  await assert.rejects(readFile(telemetryPath, "utf8"), error => error.code === "ENOENT")
...
test("complete simulated protocol pair-to-audit vertical slice", {timeout: 120_000}, async t => {
  ...
  const actions = {
    "health.request": async ({boundary}) => { const rootResponse = await fetch(world.baseUrl); assert.equal(rootResponse.ok, true); const response = await fetch(`${world.baseUrl}/health`); assert.equal(response.ok, true); assert.equal(world.manifest.manifest_version, 1); assert.equal(world.manifest.world_id, world.worldId); assert.equal(world.manifest.scenario_id, scenario.id); assert.equal(world.manifest.base_url, world.baseUrl); assert.ok(world.manifest.instance_nonce.length >= 32); assert.ok(world.manifest.artifact_directory); const proof = await new ArtifactRecorder({root: join(root, "health-artifact-proof"), scenarioId: scenario.id, worldId: `${world.worldId}-health-proof`}).open(); await proof.producers.world.event("health.boundary.verified", {status: response.status}); await proof.producers.world.artifact(world.manifestPath, {name: "world-manifest-live.json", kind: "manifest", essential: true}); const attested = await proof.finalize({status: "passed"}); assert.ok(attested.attestation.files.some(file => file.path.endsWith("events.ndjson"))); assert.ok(attested.attestation.files.some(file => file.path.endsWith("world-manifest-live.json"))); const ready = {state: "ready", value: true}; observeRecordedSurfaces(boundary, ["http:get-root", "http:get-health", "behavior:health", "capability:world-nonce", "world-field:manifest-version", "world-field:world-id", "world-field:base-url", "world-field:artifacts", "world-field:scenario-id"], "validated live root, health, and world manifest fields"); boundary.observe("artifact:timeline", surfaceProof.artifact(attested, "events.ndjson")); boundary.observe("artifact:manifest", surfaceProof.artifact(attested, "world-manifest-live.json")); boundary.complete(); return {handles: {world: world.worldId}, observations: {"health.ready": ready, "wait.shared-vertical-slice.health": ready}} },
    "browser.pair": async ({boundary}) => { await browser.connect(); observeRecordedSurfaces(boundary, ["topic:pairing"], `joined ${browser.topic}`); const approvedPush = browser.waitFor("pairing.approved"); const pending = await browser.pair({displayName: "Scenario Simulator"}); observeRecordedSurfaces(boundary, ["in:pairing-request", "out:pairing-pending"], `pairing request ${pending.pairing_id} returned pending`); const pendingStatus = await browser.pairingStatus(); assert.equal(pendingStatus.payload.status, "pending"); observeRecordedSurfaces(boundary, ["in:pairing-status", "out:pairing-status"], `pairing status ${pending.pairing_id} returned pending`); await dashboard.refresh(); browserId = await dashboard.approvePairing(pending.pairing_id, "Scenario Simulator"); const pushed = await approvedPush; assert.equal(pushed.browser_id, browserId); observeRecordedSurfaces(boundary, ["dashboard:approve", "out:pairing-approved"], `dashboard approved ${pending.pairing_id} as ${browserId}`); await browser.authenticate(browserId); assert.equal(browser.topic, "browser:auth"); observeRecordedSurfaces(boundary, ["topic:auth"], `joined ${browser.topic}`); observeRecordedSurfaces(boundary, ["out:auth-challenge", "in:auth-respond", "out:auth-accepted", "in:browser-hello", "out:browser-welcome"], `authenticated browser ${browserId} through signed challenge and welcome`); const authenticated = {state: "recovered", value: true}; boundary.complete(); return {handles: {pairing: pending.pairing_id, browser: browserId}, observations: {"browser.authenticated": authenticated, "wait.shared-vertical-slice.pair": {state: "succeeded", terminal: true, value: browserId}}} },
    "discovery.publish": async ({boundary}) => { observation = browser.observation(42, {origin: "https://scenario.fixture", toolCount: 1}); const resync = await browser.resync([observation]); assert.equal(resync.payload.observation_count, 1); observeRecordedSurfaces(boundary, ["in:browser-resync", "out:ack"], `browser.resync acknowledged ${resync.payload.observation_count} observation`); const observed = await browser.observe([observation]); assert.equal(observed.payload.observation_count, 1); observeRecordedSurfaces(boundary, ["in:discovery-observed"], `discovery.observed acknowledged document ${observation.document_id}`); await dashboard.refresh(); const row = await dashboard.rowByText("discoveries", "discovery", "Fixture 42"); discoveryId = (await row.getAttribute("id")).slice("discovery-".length); registrationId = await dashboard.registerDiscovery(discoveryId, "Fixture 42"); observeRecordedSurfaces(boundary, ["dashboard:register"], `dashboard registered discovery ${discoveryId} as ${registrationId}`); await browser.observe([observation]); await dashboard.refresh(); await dashboard.registrationSessionCount(registrationId, 1); const available = {state: "present", value: registrationId}; boundary.complete(); return {handles: {page: registrationId, document: observation.document_id, session: `${browserId}:${observation.tab_id}:${observation.document_id}`}, observations: {"page.available": available, "wait.shared-vertical-slice.discover": available}} },
    "credential.create": async ({boundary}) => { credentialLease = await dashboard.acquireCredential("call"); credentialId = credentialLease.id; observeRecordedSurfaces(boundary, ["dashboard:create-credential"], "dashboard created call credential"); boundary.complete(); return {handles: {credential: credentialId}, observations: {"wait.shared-vertical-slice.credential": {state: "present", value: true}}} },
    "mcp.invoke": async ({handles, boundary}) => credentialLease.use(async secret => {
      ...
      for (const version of ["2026-07-28", "2025-11-25", "2025-06-18", "2025-03-26"]) { const client = new MCPClient({baseUrl: world.baseUrl, token: secret, version}); try { assert.equal((await client.initialize()).status, 200) } finally { client.close() } }
      mcp = new MCPClient({baseUrl: world.baseUrl, token: secret, version: "2025-06-18", recorder: {record: recorder.producers.mcp.event}}); assert.equal((await mcp.initialize()).status, 200); assert.equal((await mcp.listTools()).status, 200); assert.equal((await mcp.call({action: "status"})).status, 200); assert.equal((await mcp.call({action: "browser.list"})).status, 200); assert.equal((await mcp.call({action: "discovery.list"})).status, 200); assert.equal((await mcp.call({action: "discovery.get", params: {discovery: discoveryId}})).status, 200)
      const pages = await mcp.call({action: "page.list"}); const pageList = pages.body.result.structuredContent ?? JSON.parse(pages.body.result.content[0].text); const registered = pageList.find(item => item.id === registrationId); assert.equal(registered.available, true); assert.equal((await mcp.call({action: "page.get", params: {page: registrationId}})).status, 200)
      ...
      assert.deepEqual(callResponse.body.result.structuredContent, expectedResult); observeRecordedSurfaces(boundary, ["http:post-mcp", "in:tool-result", "out:tool-call", "mcp:initialize", "mcp:tools-list", "mcp:tools-call", "mcp:initialized", "action:status", "action:browser-list", "action:discovery-list", "action:discovery-get", "action:page-list", "action:page-get", "action:page-tools", "action:page-call", "version:2026", "version:2025-11", "version:2025-06", "version:2025-03", "fixture:side-effect"], "live protocol initialization, list/get actions, invocation, result and fixture effect completed"); boundary.complete()
    ...
    "audit.observe": async ({boundary}) => { const audits = await sqlite(world.databasePath, `SELECT id, credential_id, browser_id, outcome, tool_name FROM invocation_audits WHERE credential_id='${credentialId}'`); assert.equal(audits.length, 1); assert.equal(audits[0].browser_id, browserId); assert.equal(audits[0].outcome, "succeeded"); assert.equal(audits[0].tool_name, "tool_0"); auditId = audits[0].id; const dashboardArtifact = await chromium.screenshot(page, "dashboard-snapshot.png"); assert.ok(dashboardArtifact?.sha256 && !dashboardArtifact.omitted); const proof = await new ArtifactRecorder({root: join(root, "audit-artifact-proof"), scenarioId: scenario.id, worldId: `${world.worldId}-audit-proof`}).open(); await proof.producers.world.artifact(world.stdoutPath, {name: "server-stdout-live.log", kind: "log", essential: true}); await proof.producers.chromium.artifact(dashboardArtifact.staged, {name: "dashboard-snapshot.png", kind: "screenshot", essential: true}); const attested = await proof.finalize({status: "passed"}); assert.ok(attested.attestation.files.some(file => file.path.endsWith("server-stdout-live.log"))); assert.ok(attested.attestation.files.some(file => file.path.endsWith("dashboard-snapshot.png"))); boundary.observe("artifact:server-log", surfaceProof.artifact(attested, "server-stdout-live.log")); boundary.observe("artifact:dashboard", surfaceProof.artifact(attested, "dashboard-snapshot.png")); boundary.complete(); return {handles: {audit: auditId}, observations: {"audit.once": {state: "present", value: 1}, "wait.shared-vertical-slice.audit": {state: "present", value: true}}} },
  }
  const observe = async () => ({})
  const cleanup = async () => {
