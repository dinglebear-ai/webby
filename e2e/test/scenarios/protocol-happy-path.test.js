...
test("complete simulated protocol pair-to-audit vertical slice", {timeout: 120_000}, async t => {
  ...
  const actions = {
    "health.request": async ({boundary}) => { const rootResponse = await fetch(world.baseUrl); assert.equal(rootResponse.ok, true); const response = await fetch(`${world.baseUrl}/health`); assert.equal(response.ok, true); assert.equal(world.manifest.manifest_version, 1); assert.equal(world.manifest.world_id, world.worldId); assert.equal(world.manifest.scenario_id, scenario.id); assert.equal(world.manifest.base_url, world.baseUrl); assert.ok(world.manifest.instance_nonce.length >= 32); assert.ok(world.manifest.artifact_directory); const proof = await new ArtifactRecorder({root: join(root, "health-artifact-proof"), scenarioId: scenario.id, worldId: `${world.worldId}-health-proof`}).open(); await proof.producers.world.event("health.boundary.verified", {status: response.status}); await proof.producers.world.artifact(world.manifestPath, {name: "world-manifest-live.json", kind: "manifest", essential: true}); const attested = await proof.finalize({status: "passed"}); assert.ok(attested.attestation.files.some(file => file.path.endsWith("events.ndjson"))); assert.ok(attested.attestation.files.some(file => file.path.endsWith("world-manifest-live.json"))); const ready = {state: "ready", value: true}; observeVerifiedSurfaces(boundary, ["http:get-root", "http:get-health", "behavior:health", "capability:world-nonce", "world-field:manifest-version", "world-field:world-id", "world-field:base-url", "world-field:artifacts", "world-field:scenario-id"], "validated live root, health, and world manifest fields"); observeVerifiedSurfaces(boundary, ["artifact:timeline", "artifact:manifest"], `artifact attestation ${attested.attestation.attestation_sha256}`); boundary.complete(); return {handles: {world: world.worldId}, observations: {"health.ready": ready, "wait.shared-vertical-slice.health": ready}} },
    "browser.pair": async ({boundary}) => { await browser.connect(); observeVerifiedSurfaces(boundary, ["topic:pairing"], `joined ${browser.topic}`); const approvedPush = browser.waitFor("pairing.approved"); const pending = await browser.pair({displayName: "Scenario Simulator"}); observeVerifiedSurfaces(boundary, ["in:pairing-request", "out:pairing-pending"], `pairing request ${pending.pairing_id} returned pending`); const pendingStatus = await browser.pairingStatus(); assert.equal(pendingStatus.payload.status, "pending"); observeVerifiedSurfaces(boundary, ["in:pairing-status", "out:pairing-status"], `pairing status ${pending.pairing_id} returned pending`); await dashboard.refresh(); browserId = await dashboard.approvePairing(pending.pairing_id, "Scenario Simulator"); const pushed = await approvedPush; assert.equal(pushed.browser_id, browserId); observeVerifiedSurfaces(boundary, ["dashboard:approve", "out:pairing-approved"], `dashboard approved ${pending.pairing_id} as ${browserId}`); await browser.authenticate(browserId); assert.equal(browser.topic, "browser:auth"); observeVerifiedSurfaces(boundary, ["topic:auth"], `joined ${browser.topic}`); observeVerifiedSurfaces(boundary, ["out:auth-challenge", "in:auth-respond", "out:auth-accepted", "in:browser-hello", "out:browser-welcome"], `authenticated browser ${browserId} through signed challenge and welcome`); const authenticated = {state: "recovered", value: true}; boundary.complete(); return {handles: {pairing: pending.pairing_id, browser: browserId}, observations: {"browser.authenticated": authenticated, "wait.shared-vertical-slice.pair": {state: "succeeded", terminal: true, value: browserId}}} },
    "discovery.publish": async ({boundary}) => { observation = browser.observation(42, {origin: "https://scenario.fixture", toolCount: 1}); const resync = await browser.resync([observation]); assert.equal(resync.payload.observation_count, 1); observeVerifiedSurfaces(boundary, ["in:browser-resync", "out:ack"], `browser.resync acknowledged ${resync.payload.observation_count} observation`); const observed = await browser.observe([observation]); assert.equal(observed.payload.observation_count, 1); observeVerifiedSurfaces(boundary, ["in:discovery-observed"], `discovery.observed acknowledged document ${observation.document_id}`); await dashboard.refresh(); const row = await dashboard.rowByText("discoveries", "discovery", "Fixture 42"); discoveryId = (await row.getAttribute("id")).slice("discovery-".length); registrationId = await dashboard.registerDiscovery(discoveryId, "Fixture 42"); observeVerifiedSurfaces(boundary, ["dashboard:register"], `dashboard registered discovery ${discoveryId} as ${registrationId}`); await browser.observe([observation]); await dashboard.refresh(); await dashboard.registrationSessionCount(registrationId, 1); const available = {state: "present", value: registrationId}; boundary.complete(); return {handles: {page: registrationId, document: observation.document_id, session: `${browserId}:${observation.tab_id}:${observation.document_id}`}, observations: {"page.available": available, "wait.shared-vertical-slice.discover": available}} },
    "credential.create": async ({boundary}) => { credentialLease = await dashboard.acquireCredential("call"); credentialId = credentialLease.id; observeVerifiedSurfaces(boundary, ["dashboard:create-credential"], "dashboard created call credential"); boundary.complete(); return {handles: {credential: credentialId}, observations: {"wait.shared-vertical-slice.credential": {state: "present", value: true}}} },
