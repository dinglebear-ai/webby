...
test("cross-world and cross-contract logical handle replay fails closed", async t => {
  ...
  assert.equal(artifact.replay.seed, 8675309)
  await emitLiveTestReceipt({scenarioId: "e2e-transport-security", adapter: "protocol", receiptId: "transport-security-live", assertions: {cross_world_replays_rejected: 2, cross_contract_replays_rejected: 2, cleanup_audits: 0}, producerRecords: [producerRecord("artifact_attestation", "artifact-recorder", "transport-security", {attestation_sha256: artifact.attestation.attestation_sha256, seed: artifact.replay.seed})]})
})
